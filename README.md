...
После первой успешной синхронизации Android запоминает целевые часы. Меню `⋮`
позволяет после отдельного подтверждения забыть эту локальную привязку либо удалить
TOTP-записи с часов. «Забыть» не разрывает pairing Garmin Connect и не меняет данные
часов. Очистка не удаляет Android-записи; следующая синхронизация вернёт их на часы.
