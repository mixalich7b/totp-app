...
- Полная загрузка списка из repository допустима при старте и после явного reset.
  После успешных add/edit/delete/import UI обновляет `EntryCollection`
  инкрементально, не очищая `ListView` на время повторного чтения БД.
- Свайп строки влево открывает действия удаления и редактирования. Удаление
  сохраняет диалог подтверждения. Редактирование не показывает, не заменяет и не
  позволяет вводить secret; сохраняются существующие ID, createdAt и secret.
