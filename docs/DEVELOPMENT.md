...
В Android release logging отключён. Garmin-логи содержат только служебные метаданные
(тип сообщения, transfer ID, revision, количество и sequence), без секретов, имён
записей, checksum и payload.
...
Android unit-набор содержит 49 тестов. Детерминированные property/fuzz-сценарии
...
`d` отправляется сразу, без предварительной синхронизации, удаляет
active/staging/favorite/revision на часах и получает ACK `z`; Android repository при
этом не меняется.
...
- Kotlin stdlib 2.2.10 и AndroidX Core KTX 1.15.0 — Apache License 2.0;
