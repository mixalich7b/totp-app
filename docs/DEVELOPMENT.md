...
- `TotpCodeCache` переиспользует рассчитанный код записи внутри одного TOTP
  time-step, не копируя secret.
...
Для UI полная загрузка decrypted records выполняется при старте приложения и после
явного reset локального storage. Перед запрошенной пользователем синхронизацией
отдельно читается свежий snapshot записей и revision. После успешных add, edit,
delete и import `EntryCollection` изменяется инкрементально, поэтому `ListView` не
очищается на время повторного чтения БД. Persistence при этом завершается до
изменения UI.

Секундный ticker `MainScreen` работает только с `EntryCollection` и не обращается
к БД. Он обновляет progress line и warning color видимых строк, а рассчитанный
TOTP-код получает через `TotpCodeCache`. Кэш выполняет HMAC при первом показе,
смене экземпляра записи или TOTP time-step и переиспользует код на остальных
тиках периода. При изменении списка кэш удаляет результаты заменённых и удалённых
записей, а при отсоединении UI очищается полностью.
