...
При потере Android Keystore key локальные данные не восстанавливаются. При потере часов или телефона следует отозвать/перевыпустить соответствующие TOTP credentials у каждого issuer.
